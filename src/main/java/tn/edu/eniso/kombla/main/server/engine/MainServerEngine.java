 ...
 */
package tn.edu.eniso.kombla.main.server.engine;
 ...
 */
@AtomSceneEngine(id = "mainServer", width = 12, height = 12)
...
//        dal = new TCPMainServerDAO();
    ...
    public Sprite createBomb(int playerId) {
        ...
        bomb.setSize(new ModelDimension(0.5, 0.5));
        bomb.setSpeed(0);
    ...
    private void move(int playerId, Orientation direction) {
        ...
        if (person != null) {
            if (!(getSpriteTask(person) instanceof MoveSpriteTask)) {
                setSpriteTask(person, new MoveSpriteTask(false));
    ...
    /**
     * each frame reevaluate Phase.
     * This method is called by ATOM to UPDATE model (R/W mode)
     */
    @Override
    ...
    /**
     * each frame broadcast shared data to players.
     * This method is called by ATOM to READ model (R/O mode)
     */
    @Override
